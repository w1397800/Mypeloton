...
#include "storage/peloton/catalog/settings_catalog.h"
...
void SettingsManager::SetInt(SettingId id, int32_t value) {
  GetInstance().SetValue(id, ValueFactory::GetIntegerValue(value));
}

void SettingsManager::SetBool(SettingId id, bool value) {
  GetInstance().SetValue(id, ValueFactory::GetBooleanValue(value));
}

void SettingsManager::SetString(SettingId id, const std::string &value) {
  GetInstance().SetValue(id, ValueFactory::GetVarcharValue(value));
}
...
void SettingsManager::InitializeCatalog() {
  auto &settings_catalog = SettingsCatalog::GetInstance();

  auto &txn_manager = TransactionManagerFactory::GetInstance();
  auto txn = txn_manager.BeginTransaction();
  AbstractPool *pool = pool_.get();
...
void SettingsManager::DefineSetting(SettingId id, const std::string &name,
  ...
  if (value.GetTypeId() == TypeId::INTEGER ||
      value.GetTypeId() == TypeId::SMALLINT ||
      value.GetTypeId() == TypeId::TINYINT ||
      value.GetTypeId() == TypeId::DECIMAL) {
...
Value SettingsManager::GetValue(SettingId id) {
...
void SettingsManager::SetValue(SettingId id, const Value &value) {
  auto param = settings_.find(id);
