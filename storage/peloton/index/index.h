...
class IndexMetadata  {
 public:
  IndexMetadata(std::string index_name, uint index_oid, uint table_oid,
                uint database_oid, IndexType index_type,
                IndexConstraintType index_constraint_type,
                const Schema *tuple_schema,
                const Schema *key_schema,
                const std::vector<uint> &key_attrs, bool unique_keys);

  ~IndexMetadata();

  const std::string &GetName() const { return name_; }

  uint GetOid() const { return index_oid; }

  uint GetTableOid() const { return table_oid; }

  uint GetDatabaseOid() const { return database_oid; }
  ...
  const Schema *GetKeySchema() const { return key_schema; }

  // Returns a schema object that represents the schema of the underlying table
  const Schema *GetTupleSchema() const { return tuple_schema; }
  ...
  const std::vector<uint> &GetKeyAttrs() const { return key_attrs; }

  // Returns the mapping relation between tuple key column and index key columns
  const std::vector<uint> &GetTupleToIndexMapping() const {
    return tuple_attrs;
  }
...
//  const std::string GetInfo() const override;
  ...
  // deprecated, use IndexCatalog::GetInstance()->GetIndexName()
  std::string name_;

  const uint index_oid;
  const uint table_oid;
...
//  friend class IndexFactory;
 ...
 public:
   ...
   */
  uint TupleColumnToKeyColumn(uint tuple_column_id) const;
   ...
   */
  virtual bool InsertEntry(const Tuple *key,
   ...
   */
  virtual bool DeleteEntry(const Tuple *key,
                           ItemPointer *location_ptr) = 0;
   ...
   * @return True on successful insertion
   */
  virtual bool CondInsertEntry(const Tuple *key, ItemPointer *location,
...
//with tid 传入mysql线程id
  ...
  //with tid 根据主键查询
  virtual void PointGet(const std::vector<Value> &value_list,
   ...
   */
  virtual void ScanLimit(const std::vector<Value> &value_list,
                         const std::vector<uint> &tuple_column_id_list,
                         const std::vector<ExpressionType> &expr_list,
                         ScanDirectionType scan_direction,
                         std::vector<ItemPointer *> &result,
   ...
   * @param[out] result Where the results of the scan are stored
   */
  virtual void ScanKey(const Tuple *key,
  ...
  uint GetColumnCount() const { return metadata->GetColumnCount(); }
...
//  const std::string GetInfo() const override;
  ...
  AbstractPool *GetPool() const { return pool; }

  uint GetKeyLength(){
    return key_length_;
  }

  void SetKeyLength(uint len){
    key_length_ = len;
  }
