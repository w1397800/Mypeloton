...
//            metadata->key_schema->GetIndexedColumns().size());
  ...
  for (const auto &column : metadata->key_schema->GetColumns()) {
    ...
    if (col_type != TypeId::TINYINT &&
        col_type != TypeId::SMALLINT &&
        col_type != TypeId::INTEGER && col_type != TypeId::BIGINT) {
      ints_only = false;
      break;
    }
  }  // FOR
...
//      index = IndexFactory::GetBwTreeGenericKeyIndex(metadata);
  ...
  }else if(index_type == IndexType::MASSTREE){
    index = new MassTreeIndex(metadata);
  }
