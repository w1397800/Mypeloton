...
#include "storage/peloton/util/string_util.h"
#include "storage/peloton/util/portable_endian.h"
...
class CompactIntsKey {
 ...
 private:
   ...
   * get data
   */
  inline size_t SetFromColumn(uint key_column_id, uint tuple_column_id,
                              const Schema *key_schema,
                              const Tuple *tuple, size_t offset) {
 ...
 public:
  ...
  inline void SetFromKey(const Tuple *tuple) {
    ...
    const Schema *key_schema = tuple->GetSchema();

    // Need this to loop through columns
    uint column_count = key_schema->GetColumnCount();
   ...
   */
  inline void SetFromTuple(const Tuple *tuple, const int *indices,
                           const Schema *key_schema) {
    PELOTON_ASSERT(tuple != nullptr);
    ...
    for (uint key_column_id = 0; key_column_id < column_count;
         key_column_id++) {
      // indices array maps key column to tuple column
      // and it must have the same length as key schema
      uint tuple_column_id = indices[key_column_id];
    ...
    for (uint column_id = 0; column_id < column_count; column_id++) {
      ...
      switch (column_type) {
        case TypeId::BIGINT: {
          ...
          tuple.SetValue(column_id, ValueFactory::GetBigIntValue(data));
        ...
        case TypeId::INTEGER: {
          ...
          tuple.SetValue(column_id, ValueFactory::GetIntegerValue(data));
        ...
        case TypeId::SMALLINT: {
          ...
          tuple.SetValue(column_id, ValueFactory::GetSmallIntValue(data));
        ...
        case TypeId::TINYINT: {
          ...
          tuple.SetValue(column_id, ValueFactory::GetTinyIntValue(data));
