...
//#include "storage/peloton/statistics/stats_aggregator.h"//不需要
 ...
 */
BWTREE_TEMPLATE_ARGUMENTS
bool BWTREE_INDEX_TYPE::InsertEntry(const Tuple *key,
                                    ItemPointer *value) {
  KeyType index_key;
...
//  if (static_cast<StatsType>(SettingsManager::GetInt(SettingId::stats_mode)) != StatsType::INVALID) {
//    BackendStatsContext::GetInstance()->IncrementIndexInserts(metadata);
//  }
 ...
 */
BWTREE_TEMPLATE_ARGUMENTS
bool BWTREE_INDEX_TYPE::DeleteEntry(const Tuple *key,
                                    ItemPointer *value) {
  KeyType index_key;
  index_key.SetFromKey(key);
...
//  if (static_cast<StatsType>(SettingsManager::GetInt(SettingId::stats_mode)) != StatsType::INVALID) {
//    BackendStatsContext::GetInstance()->IncrementIndexDeletes(
...
bool BWTREE_INDEX_TYPE::CondInsertEntry(
    const Tuple *key, ItemPointer *value,
...
BWTREE_TEMPLATE_ARGUMENTS
bool BWTREE_INDEX_TYPE::CondInsertEntry(const Tuple *key, ItemPointer *value,
                            std::function<bool(const void *)> predicate,
                            uint curr_thd_id) {
  (void)curr_thd_id;
  KeyType index_key;
...
void BWTREE_INDEX_TYPE:: Scan(const std::vector<Value> &value_list,
  ...
  if (csp_p->IsPointQuery() == true) {
    const Tuple *point_query_key_p = csp_p->GetPointQueryKey();
    ...
    for (auto scan_itr = container.Begin(); (scan_itr.IsEnd() == false);
         scan_itr++) {
      result.push_back(scan_itr->second);//todo 潜在的瓶颈.
    }  // for it from begin() to end()
  } else {
    const Tuple *low_key_p = csp_p->GetLowKey();
    const Tuple *high_key_p = csp_p->GetHighKey();
...
//  if (static_cast<StatsType>(SettingsManager::GetInt(SettingId::stats_mode)) != StatsType::INVALID) {
//    BackendStatsContext::GetInstance()->IncrementIndexReads(
...
void BWTREE_INDEX_TYPE::Scan(
    UNUSED_ATTRIBUTE const std::vector<Value> &value_list,
    UNUSED_ATTRIBUTE const std::vector<uint> &tuple_column_id_list,
...
BWTREE_TEMPLATE_ARGUMENTS
void BWTREE_INDEX_TYPE::ScanKey(const Tuple *key,
