...
class TupleKey {
 ...
 public:
  ...
  inline void SetFromTuple(const Tuple *tuple, const int *indices,
                           UNUSED_ATTRIBUTE const Schema *key_schema) {
  ...
  const Tuple GetTupleForComparison(
      const Schema *key_tuple_schema) const {
    return Tuple(key_tuple_schema, key_tuple);
  }
 ...
 */
  const std::string GetInfo() const {
    Tuple tuple(key_tuple_schema, key_tuple);
...
struct TupleKeyHasher {
  ...
  inline size_t operator()(const TupleKey &p) const {
    Tuple pTuple = p.GetTupleForComparison(p.key_tuple_schema);
...
class TupleKeyComparator {
 public:
  ...
  inline bool operator()(const TupleKey &lhs, const TupleKey &rhs) const {
    ...
    Tuple lhTuple = lhs.GetTupleForComparison(lhs.key_tuple_schema);
    Tuple rhTuple = rhs.GetTupleForComparison(rhs.key_tuple_schema);
