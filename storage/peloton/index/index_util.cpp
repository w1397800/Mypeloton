...
bool IndexUtil::FindValueIndex(const IndexMetadata *metadata_p,
  ...
  for(uint i = 0;i < tuple_column_id_list.size();i++) {
    
    // This is only used to retrieve index column id
    uint tuple_column_id = tuple_column_id_list[i];

    // Map tuple column to index column
    uint index_column_id = \
 ...
 */
bool IndexUtil::ValuePairComparator(const std::pair<Value, int> &i,
...
void IndexUtil::ConstructIntervals(uint leading_column_id,
  ...
  if (nums[0].second < 0) {
    ...
    cur = Type::GetMinValue(nums[0].first.GetTypeId());
  }
  ...
  while (i < nums.size()) {
    if (nums[i].second > 0) {
      if (i + 1 < nums.size() && nums[i + 1].second < 0) {
        ...
        intervals.push_back(std::pair<Value, Value>(
          cur, nums[i].first));
        cur = ValueFactory::GetNullValueByType(nums[0].first.GetTypeId());
      }
    }
    i++;
  }
  ...
  if (cur.IsNull() == false) {
    intervals.push_back(std::pair<Value, Value>(
        cur, Type::GetMaxValue(nums[0].first.GetTypeId())));
  }

  // Finish invtervals building.
};

void IndexUtil::FindMaxMinInColumns(uint leading_column_id,
                         const std::vector<Value> &values,
                         const std::vector<uint> &key_column_ids,
                         const std::vector<ExpressionType> &expr_types,
                         std::map<uint, std::pair<Value,
                             Value>> &non_leading_columns) {
  // find extreme nums on each column.
  // LOG_TRACE("FindMinMax leading column %d\n", leading_column_id);
  for (size_t i = 0; i < key_column_ids.size(); i++) {
    uint column_id = key_column_ids[i];
    ...
    if (non_leading_columns.find(column_id) == non_leading_columns.end()) {
      ...
      // std::pair<uint, std::pair<Value, Value>> key_value(column_id, range);
      non_leading_columns.insert(std::pair<uint,
        std::pair<Value, Value>>(
          column_id, std::pair<Value, Value>(
            ValueFactory::GetNullValueByType(type),
            ValueFactory::GetNullValueByType(type))));
  ...
  for (const auto &k_v : non_leading_columns) {
    if (k_v.second.first.IsNull()) {
      non_leading_columns[k_v.first].first =
          Type::GetMinValue(k_v.second.first.GetTypeId());
    }
    if (k_v.second.second.IsNull()) {
      non_leading_columns[k_v.first].second =
          Type::GetMinValue(k_v.second.second.GetTypeId());
    }
  }
}
