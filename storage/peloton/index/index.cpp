...
uint IndexMetadata::GetColumnCount() const {
...
IndexMetadata::IndexMetadata(std::string index_name, uint index_oid,
                             uint table_oid, uint database_oid,
...
//
// This function accepts an uint which must be in the range of table column
// ID, and returns a uint which is also inside the range of key columns.
//
...
uint Index::TupleColumnToKeyColumn(uint tuple_column_id) const {
  // This stores the mapping
  const std::vector<uint> &mapping = metadata->GetTupleToIndexMapping();

  // First check whether the table column ID is valid
  PELOTON_ASSERT(tuple_column_id < mapping.size());
  uint key_column_id = mapping[tuple_column_id];
...
bool Index::Compare(const AbstractTuple &index_key,
  ...
  uint cond_num = tuple_column_id_list.size();
  ...
  // comparion operand
  const std::vector<uint> &tuple_to_index_map =
  ...
  for (uint i = 0; i < cond_num; i++) {
    // First retrieve the tuple column ID from the map, and then map
    // it to the column ID of index key
    uint tuple_key_column_id = tuple_column_id_list[i];
    uint index_key_column_id = tuple_to_index_map[tuple_key_column_id];
    ...
    Value val = (index_key.GetValue(index_key_column_id));
    const Value &lhs = val;
...
//bool Index::CondInsertEntry(const Tuple *key, ItemPointer *location,
