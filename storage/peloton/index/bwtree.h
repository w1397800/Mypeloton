...
// PELOTON_ASSERT(it == it2);
  ...
  InnerNode *CollectAllSepsOnInner(NodeSnapshot *snapshot_p, int p_depth = 0) {
    ...
    const InnerDataNode **data_node_list = new const InnerDataNode*[delta_record_num];
  ...
  void NavigateLeafNode(Context *context_p,
    ...
    // a stack overflow
    const ValueType **present_set_data_p = new const ValueType*[set_max_size];
    const ValueType **deleted_set_data_p = new const ValueType*[set_max_size];
  ...
  LeafNode *CollectAllValuesOnLeaf(NodeSnapshot *snapshot_p,
    ...
    const KeyValuePair **delta_set_data_p = new const KeyValuePair*[delta_change_num];
    ...
    const LeafDataNode **sss_data_p = new const LeafDataNode*[delta_change_num];
...
// constructor
  ...
  inline NodeID FindLeftSibling(const KeyType &search_key,
    ...
    const InnerDataNode **data_node_list = new const InnerDataNode*[node_p->GetDepth()];
...
// freed has been called inside epoch manager
    ...
    ~EpochManager() {
      ...
      if (head_epoch_p != nullptr) {
        ...
        // LOG_DEBUG("%s", GETINFO_THICK_LINE.c_str());
        // LOG_DEBUG("DUMP");
