...
class ConjunctionScanPredicate {
 private:
  ...
  // index key
  std::vector<std::pair<uint, uint>> value_index_list_;
  ...
  //
  // The element is a pair of uint. The first uint is the index
  // in index key that needs a binding, and the second uint is the
  // index inside the new Value array during late binding
  std::vector<std::pair<uint, uint>> low_key_bind_list_;
  std::vector<std::pair<uint, uint>> high_key_bind_list_;
 ...
 private:
  ...
  void SetTupleColumnValueForKey(Index *index_p,
                                 const std::vector<uint> &column_id_list,
                                 const std::vector<Value> &new_value_list,
                                 Tuple *key_p) {
    ...
    int i = 0;
    for (uint tuple_column : column_id_list) {
      uint index_column = index_p->TupleColumnToKeyColumn(tuple_column);

      UNUSED_ATTRIBUTE uint bind_ret =
 ...
 public:
  ...
  ConjunctionScanPredicate(Index *index_p,
    ...
    if (full_index_scan_ == false) {
      ...
      low_key_p_ = new Tuple(metadata_p->GetKeySchema(), true);
      high_key_p_ = new Tuple(metadata_p->GetKeySchema(), true);
//      low_key_p_ = new Tuple(metadata_p->GetKeySchema(), false);
//      high_key_p_ = new Tuple(metadata_p->GetKeySchema(), false);
    ...
    key_length_ = index_p->GetKeyLength();
    return;
  }
  ...
  ConjunctionScanPredicate(Index *index_p,
                           const std::vector<Value> &value_list,
                           const std::vector<uint> &tuple_column_id_list,
                           const std::vector<ExpressionType> &expr_list, bool isfullScan) {
    // It contains a pointer to the index key schema
    IndexMetadata *metadata_p = index_p->GetMetadata();
...
//      high_key_p_ = new Tuple(metadata_p->GetKeySchema(), false);
  ...
  ConjunctionScanPredicate(const ConjunctionScanPredicate &other) {
    ...
    low_key_p_ = new Tuple(*(other.low_key_p_));
    high_key_p_ = new Tuple(*(other.high_key_p_));
  }
  ...
  inline void SetTupleColumnValueForLowKey(
      Index *index_p, const std::vector<uint> &column_id_list,
      const std::vector<Value> &new_value_list) {
   ...
   */
  static uint BindValueToIndexKey(Index *index_p, const Value &value,
                                   Tuple *index_key_p, uint index) {
    TypeId bind_type = value.GetTypeId();

    if (bind_type == TypeId::PARAMETER_OFFSET) {
      return static_cast<uint>(ValuePeeker::PeekParameterOffset(value));
    }

    // This is the type of the actual column
    TypeId column_type = index_key_p->GetType(index);
    ...
    if (column_type == bind_type) {
      ...
      Value casted_val = value.CastAs(column_type);
...
//
    ...
    // corresponding type
    for (uint i = 0; i < value_index_list_.size(); i++) {
      const std::pair<uint, uint> &index_pair = value_index_list_[i];

      // We use the type of the current index key column to get the
      // +Inf, -Inf and/or casted type for Value object
      TypeId index_key_column_type =
      ...
      if (index_pair.first == INVALID_OID) {
        ...
        Value val = (Type::GetMinValue(index_key_column_type));
        low_key_p_->SetValue(i, val, index_p->GetPool());
      } else {
        uint bind_ret = BindValueToIndexKey(
      ...
      if (is_point_query_ == false) {
        if (index_pair.second == INVALID_OID) {
          // We set the value using index's varlen pool, if any VARCHAR is
          // involved (this is OK since the routine only runs for once)
          Value val(Type::GetMaxValue(index_key_column_type));
  ...
  void LateBind(Index *index_p, const std::vector<Value> &value_list,
                const std::vector<std::pair<uint, uint>> key_bind_list,
                Tuple *index_key_p) {
   ...
   */
  inline const Tuple *GetLowKey() const {
    PELOTON_ASSERT(is_point_query_ == false);
    PELOTON_ASSERT(full_index_scan_ == false);
   ...
   * for a point query
   */
  inline const Tuple *GetHighKey() const {
    PELOTON_ASSERT(is_point_query_ == false);
    PELOTON_ASSERT(full_index_scan_ == false);
   ...
   */
  inline const Tuple *GetPointQueryKey() const {
  ...
  inline uint GetKeyLength() const{
    return key_length_;
  }

  inline uint SetKeyLength(uint len) {
    key_length_ = len;
  }
};
