...
void Schema::CreateTupleSchema(
  ...
  uint num_columns = column_types.size();
  uint column_offset = 0;

  for (uint column_itr = 0; column_itr < num_columns; column_itr++) {
...
Schema::Schema(const std::vector<Column> &columns)
  ...
  uint column_count = columns.size();

  std::vector<TypeId> column_types;
  std::vector<uint> column_lengths;
...
Schema *Schema::CopySchema(const Schema *schema,
  ...
  for (uint column_index : index_list) {
...
Schema *Schema::AppendSchema(const Schema *first, std::vector<uint> &first_set,
                             const Schema *second,
                             std::vector<uint> &second_set) {
  const std::vector<const Schema *> schema_list({first, second});
  const std::vector<std::vector<uint>> subsets({first_set, second_set});
...
//  PELOTON_ASSERT(schema_list.size() == subsets.size());
  ...
  for (unsigned int i = 0; i < schema_list.size(); i++) {
    ...
    const std::vector<uint> &subset = subsets[i];
...
bool Schema::operator==(const Schema &other) const {
  ...
  for (uint column_itr = 0; column_itr < other.GetColumnCount();
       column_itr++) {
