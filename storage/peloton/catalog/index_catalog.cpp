...
IndexCatalogEntry::IndexCatalogEntry(LogicalTile *tile, int tupleId)
...
IndexCatalog::IndexCatalog(TransactionContext *,
                           Database *pg_catalog,
                           AbstractPool *)
 ...
 */
std::unique_ptr<Schema> IndexCatalog::InitializeSchema() {
  auto index_id_column = Column(
      TypeId::INTEGER, Type::GetTypeSize(TypeId::INTEGER),
      "index_oid", true);
  index_id_column.SetNotNull();

  auto index_name_column = Column(TypeId::VARCHAR, max_name_size_,
                                           "index_name", false);
  ...
  auto index_type_column = Column(
      TypeId::INTEGER, Type::GetTypeSize(TypeId::INTEGER),
      "index_type", true);
  index_type_column.SetNotNull();

  auto index_constraint_column = Column(
      TypeId::INTEGER, Type::GetTypeSize(TypeId::INTEGER),
      "index_constraint", true);
  index_constraint_column.SetNotNull();

  auto unique_keys = Column(
      TypeId::BOOLEAN, Type::GetTypeSize(TypeId::BOOLEAN),
      "unique_keys", true);
  unique_keys.SetNotNull();

  auto indexed_attributes_column = Column(
      TypeId::VARCHAR, max_name_size_, "indexed_attributes", false);
  indexed_attributes_column.SetNotNull();


  std::unique_ptr<Schema> index_schema(new Schema(
  ...
  index_schema->AddConstraint(std::make_shared<Constraint>(
      INDEX_CATALOG_CON_PKEY_OID, ConstraintType::PRIMARY, "con_primary",
      INDEX_CATALOG_OID, std::vector<uint>{ColumnId::INDEX_OID},
      INDEX_CATALOG_PKEY_OID));

  index_schema->AddConstraint(std::make_shared<Constraint>(
      INDEX_CATALOG_CON_UNI0_OID, ConstraintType::UNIQUE, "con_unique",
      INDEX_CATALOG_OID, std::vector<uint>{ColumnId::INDEX_NAME, ColumnId::SCHEMA_NAME},
      INDEX_CATALOG_SKEY0_OID));

  return index_schema;
}

bool IndexCatalog::InsertIndex(TransactionContext *txn,
  ...
  auto val0 = ValueFactory::GetIntegerValue(index_oid);
  auto val1 = ValueFactory::GetVarcharValue(index_name, nullptr);
  auto val2 = ValueFactory::GetIntegerValue(table_oid);
  auto val3 = ValueFactory::GetVarcharValue(schema_name, nullptr);
  auto val4 = ValueFactory::GetIntegerValue(static_cast<int>(index_type));
  auto val5 =
      ValueFactory::GetIntegerValue(static_cast<int>(index_constraint));
  auto val6 = ValueFactory::GetBooleanValue(unique_keys);

  std::stringstream os;
  for (uint indkey : index_keys) os << std::to_string(indkey) << " ";
  auto val7 = ValueFactory::GetVarcharValue(os.str(), nullptr);
...
bool IndexCatalog::DeleteIndex(TransactionContext *txn,
                               uint database_oid,
                               uint index_oid) {
  uint index_offset = IndexId::PRIMARY_KEY;  // Index of index_oid
  std::vector<Value> values;
  values.push_back(ValueFactory::GetIntegerValue(index_oid).Copy());
...
std::shared_ptr<IndexCatalogEntry> IndexCatalog::GetIndexCatalogEntry(
  ...
  std::vector<uint> column_ids(all_column_ids);
  uint index_offset = IndexId::PRIMARY_KEY;  // Index of index_oid
  std::vector<Value> values;
  values.push_back(ValueFactory::GetIntegerValue(index_oid).Copy());
...
std::shared_ptr<IndexCatalogEntry> IndexCatalog::GetIndexCatalogEntry(
  ...
  std::vector<Value> values;
  values.push_back(
      ValueFactory::GetVarcharValue(index_name, nullptr).Copy());
  values.push_back(
