...
TableCatalogEntry::TableCatalogEntry(TransactionContext *txn,
                                       LogicalTile *tile,
                                       int tupleId)
    : table_oid(tile->GetValue(tupleId, TableCatalog::ColumnId::TABLE_OID)
      ...
      default_layout_oid(tile->GetValue(tupleId,
      		TableCatalog::ColumnId::DEFAULT_LAYOUT_OID).GetAs<uint>()),
 ...
 */
bool TableCatalogEntry::EvictIndexCatalogEntry(uint index_oid) {
...
std::shared_ptr<IndexCatalogEntry> TableCatalogEntry::GetIndexCatalogEntries(
    uint index_oid, bool cached_only) {
...
bool TableCatalogEntry::InsertColumnCatalogEntry(
  ...
  if (column_catalog_entries_.find(column_catalog_entry->GetColumnId()) !=
    ...
    //_DEBUG("Column %u already exists in cache!",
 ...
 */
bool TableCatalogEntry::EvictColumnCatalogEntry(uint column_id) {
...
std::shared_ptr<ColumnCatalogEntry> TableCatalogEntry::GetColumnCatalogEntry(
    uint column_id, bool cached_only) {
...
TableCatalog::TableCatalog(TransactionContext *,
                           Database *database,
                           AbstractPool *)
 ...
 */
std::shared_ptr<const Layout> TableCatalogEntry::GetLayout(
    uint layout_id, bool cached_entry) {
 ...
 */
bool TableCatalogEntry::EvictLayout(uint layout_id) {
...
bool TableCatalogEntry::InsertConstraintCatalogEntry(
  ...
  uint constraint_oid = constraint_catalog_entry->GetConstraintOid();
  // layout is already present in the cache.
 ...
 */
bool TableCatalogEntry::EvictConstraintCatalogEntry(uint constraint_oid) {
...
TableCatalogEntry::GetConstraintCatalogEntry(uint constraint_oid, bool cached_only){
 ...
 */
std::unique_ptr<Schema> TableCatalog::InitializeSchema() {
  auto table_id_column = Column(
      TypeId::INTEGER, Type::GetTypeSize(TypeId::INTEGER),
      "table_oid", true);
  table_id_column.SetNotNull();

  auto table_name_column = Column(TypeId::VARCHAR, max_name_size_,
                                           "table_name", false);
  ...
  auto version_id_column = Column(
      TypeId::INTEGER, Type::GetTypeSize(TypeId::INTEGER),
      "version_id", true);
  version_id_column.SetNotNull();

  auto default_layout_id_column = Column(
      TypeId::INTEGER, Type::GetTypeSize(TypeId::INTEGER),
      "default_layout_oid", true);
  default_layout_id_column.SetNotNull();

  std::unique_ptr<Schema> table_catalog_schema(new Schema(
  ...
  table_catalog_schema->AddConstraint(std::make_shared<Constraint>(
      ...
      TABLE_CATALOG_OID, std::vector<uint>{ColumnId::TABLE_OID},
      TABLE_CATALOG_PKEY_OID));

  table_catalog_schema->AddConstraint(std::make_shared<Constraint>(
      TABLE_CATALOG_CON_UNI0_OID, ConstraintType::UNIQUE, "con_unique",
      TABLE_CATALOG_OID, std::vector<uint>{ColumnId::TABLE_NAME, ColumnId::SCHEMA_NAME},
      TABLE_CATALOG_SKEY0_OID));
 ...
 */
bool TableCatalog::InsertTable(TransactionContext *txn,
                               uint database_oid,
  ...
  auto val1 = ValueFactory::GetVarcharValue(table_name, nullptr);
  auto val2 = ValueFactory::GetVarcharValue(schema_name, nullptr);
  auto val3 = ValueFactory::GetIntegerValue(database_oid);
  auto val4 = ValueFactory::GetIntegerValue(0);
  auto val5 = ValueFactory::GetIntegerValue(layout_oid);
 ...
 */
bool TableCatalog::DeleteTable(TransactionContext *txn, uint table_oid) {
...
std::shared_ptr<TableCatalogEntry> TableCatalog::GetTableCatalogEntry(
  ...
  // cache miss, get from pg_table
  std::vector<uint> column_ids(all_column_ids_);
  uint index_offset = IndexId::SKEY_TABLE_NAME;  // Index of table_name
  std::vector<Value> values;
  values.push_back(
      ValueFactory::GetVarcharValue(table_name, nullptr).Copy());
  values.push_back(
...
TableCatalog::GetTableCatalogEntries(TransactionContext *txn) {
  if (txn == nullptr) {
    // throw CatalogException("Transaction is invalid!");
  }
  // try get from cache
  ...
  // cache miss, get from pg_table
  std::vector<uint> column_ids(all_column_ids_);
  uint index_offset = IndexId::SKEY_DATABASE_OID;  // Index of database_oid
  std::vector<Value> values;
  values.push_back(ValueFactory::GetIntegerValue(database_oid_).Copy());
 ...
 */
bool TableCatalog::UpdateVersionId(TransactionContext *txn,
                                   uint table_oid,
                                   uint update_val) {
  std::vector<uint> update_columns({ColumnId::VERSION_ID});  // version_id
  uint index_offset = IndexId::PRIMARY_KEY;  // Index of table_oid
  ...
  update_values.push_back(
      ValueFactory::GetIntegerValue(update_val).Copy());
 ...
 * @return  Whether update is successful
 */
bool TableCatalog::UpdateDefaultLayoutOid(TransactionContext *txn,
                                          uint table_oid,
                                          uint update_val) {
  std::vector<uint> update_columns({ColumnId::DEFAULT_LAYOUT_OID});  // defalut_layout_oid
  uint index_offset = IndexId::PRIMARY_KEY;  // Index of table_oid
  // values to execute index scan
  std::vector<Value> scan_values;
  scan_values.push_back(ValueFactory::GetIntegerValue(table_oid).Copy());
