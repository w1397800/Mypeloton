...
#pragma once
//#ifndef SCHEMA_PELOTON
//#define SCHEMA_PELOTON
#include <memory>
#include <vector>
#include "column.h"
...
public:
    ...
    // Construct schema
    void CreateTupleSchema(const std::vector<TypeId> &column_types,
                           const std::vector<uint> &column_lengths,
    ...
    static Schema *AppendSchema(const Schema *first,
                                std::vector<uint> &first_set,
                                const Schema *second,
                                std::vector<uint> &second_set);
    ...
    static Schema *AppendSchemaPtrList(
            const std::vector<const Schema *> &schema_list,
            const std::vector<std::vector<uint>> &subsets);
    ...
    inline size_t GetOffset(const uint column_id) const {
        return columns_[column_id].GetOffset();
    }

    inline TypeId GetType(const uint column_id) const {
        return columns_[column_id].GetType();
    }

    // Return appropriate length based on whether column is inlined
    inline size_t GetAppropriateLength(const uint column_id) const {
    ...
    inline size_t GetLength(const uint column_id) const {
        return columns_[column_id].GetLength();
    }

    inline size_t GetVariableLength(const uint column_id) const {
        return columns_[column_id].GetVariableLength();
    }

    inline bool IsInlined(const uint column_id) const {
        return columns_[column_id].IsInlined();
    }

    inline const Column &GetColumn(const uint column_id) const {
     ...
     * @return
     */
    inline uint GetColumnID(std::string col_name) const {
        uint index = INVALID_OID;
        for (uint i = 0, cnt = columns_.size(); i < cnt; ++i) {
    ...
    inline uint GetUninlinedColumn(const uint column_id) const {
    ...
    inline uint GetUninlinedColumnCount() const {
        return uninlined_column_count_;
    }

    // Return the number of bytes used by one tuple.
    inline uint GetLength() const { return length_; }

    // Returns a flag indicating whether all columns are inlined
    inline bool IsInlined() const { return tuple_is_inlined_; }

    inline void SetIndexedColumns(const std::vector<uint> &indexed_columns) {
        indexed_columns_ = indexed_columns;
    }

    inline const std::vector<uint> GetIndexedColumns() const {
    ...
    inline bool AllowNull(const uint column_id) const {
    ...
    inline std::vector<uint> GetNotNullColumns() const {
        return not_null_columns_;
    }

    // For single column default
    inline bool AllowDefault(const uint column_id) const {
        return columns_[column_id].HasDefault();
    }

    // Get the default value for the column
    inline Value* GetDefaultValue(const uint column_id) const {
//    inline int *GetDefaultValue(const uint column_id) const {
    ...
    inline void SetDefaultValue(const uint column_id,
...
//                                const int &default_value) {
    ...
    inline void DropDefaultValue(const uint column_id) {
        if (columns_[column_id].HasDefault()) {
            columns_[column_id].ClearDefaultValue();
        }
    }
    ...
    inline void DropConstraint(uint constraint_oid) {
    ...
    inline std::unordered_map<uint, std::shared_ptr<Constraint>> GetConstraints() const {
        return constraints;
    }

    inline std::shared_ptr<Constraint> GetConstraint(uint constraint_oid) const {
    ...
    inline bool IsPrimary(uint column_id) {
...
//            PELOTON_ASSERT(constraints[oid]->GetType() == ConstraintType::FOREIGN);
    ...
    inline void DeleteForeignKeySource(const uint constraint_oid) {
...
private:
    ...
    uint column_count_ = INVALID_OID;

    // keeps track of unlined columns
    std::vector<uint> uninlined_columns_;

    uint uninlined_column_count_ = INVALID_OID;

    // are all columns inlined
    bool tuple_is_inlined_;

    // keeps track of indexed columns in original table
    std::vector<uint> indexed_columns_;

    // Constraint Information
    // keeps constraints
    std::unordered_map<uint, std::shared_ptr<Constraint>> constraints;

    // not null column list for fast constraint checking
    std::vector<uint> not_null_columns_;

    // primary key column list for fast column check
    std::vector<uint> primary_key_columns_;

    // # of unique constraints
    uint unique_constraint_count_ = START_OID;

    // list of foreign key constraints
    std::vector<uint> fk_constraints_;
