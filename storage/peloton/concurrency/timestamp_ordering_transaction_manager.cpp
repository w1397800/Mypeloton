...
#include "storage/peloton/concurrency/timestamp_ordering_transaction_manager.h"
#include <cinttypes>
#include "common/util.h"
#include "storage/peloton/store/storage_manager.h"

#include "storage/peloton/catalog/catalog_defaults.h"
...
//#include "storage/peloton/logging/log_manager_factory.h"
...
//    sql_print_error("con1= tuple_txn_id, current_txn: %ld, %ld", tuple_txn_id, current_txn->GetTransactionId());
//    sql_print_error("con1:current_txn=%ld", current_txn->GetTransactionId());
  }

  if(tuple_end_cid != MAX_CID){
//    sql_print_error("con1:tuple_end_cid=%ld", tuple_end_cid);
  }
...
//    sql_print_error("con2:current_txn ,last_reader_cid: %ld,%ld", current_txn->GetTransactionId(),last_reader_cid);
//    sql_print_error("con2:current_txn=%ld", current_txn->GetTransactionId());
//    sql_print_error("con2:last_reader_cid=%ld", last_reader_cid);
...
TransactionManager::bool_ex TimestampOrderingTransactionManager::PerformRead(TransactionContext *const current_txn,
...
//    LOG_TRACE("PerformRead (%u, %u)\n", location.block, location.offset);
    ...
    if (acquire_ownership == true) {
      ...
      if (IsOwner(current_txn, tile_group_header, tuple_id) == false) {
        ...
        // -------------------------
        // Record RWType::READ_OWN
        // 注意这里location一定是最新的，否则之前会重新赋值或者直接abort
        current_txn->RecordReadOwn(location);

        // now we have already obtained the ownership.
        // then attempt to set last reader cid.
        // 此处不必修改，传入的is_owner参数为true。（因为AcquireOwnership部分）
...
void TimestampOrderingTransactionManager::PerformInsert(
  ...
  uint tile_group_id = location.block;
  uint tuple_id = location.offset;
...
//            new_location.offset);
  ...
  PELOTON_ASSERT(tile_group_header->GetTransactionId(old_location.offset) ==
                 transaction_id);
//  if(! tile_group_header->GetPrevItemPointer(old_location.offset).IsNull()){
//    sql_print_error("======================819=====================");
//  }
  PELOTON_ASSERT(
...
void TimestampOrderingTransactionManager::PerformUpdate(
  ...
  uint tile_group_id = location.block;
  UNUSED_ATTRIBUTE uint tuple_id = location.offset;
...
ResultType TimestampOrderingTransactionManager::CommitTransaction(
  ...
  if (!current_txn->IsReadOnly()
      && current_txn->GetResult() != ResultType::FAILURE
      && !current_txn->GetReadWriteSet().empty()) {
    log_manager.LogBeginTransaction(current_txn);
  }
  ...
  for (auto &obj : rw_object_set) {
    ...
    uint database_oid = std::get<0>(obj);
    uint table_oid = std::get<1>(obj);
    uint index_oid = std::get<2>(obj);
  ...
  uint last_tile_group_id = INVALID_OID;
  TileGroupHeader *tile_group_header = nullptr;

  // mraft::MetricTimer mtimer("Generate log record latancy");
  // mtimer.start();

  for (const auto &tuple_entry : rw_set) {
    ItemPointer item_ptr = tuple_entry.first;
    uint tile_group_id = item_ptr.block;
    uint tuple_slot = item_ptr.offset;
    ...
    if (tuple_entry.second == RWType::READ_OWN) {
      ...
      log_manager.LogDelete(
          current_txn, ItemPointer(tile_group_id, tuple_slot));

    } else if (tuple_entry.second == RWType::INSERT) {
//      txn_id_t tid123 = tile_group_header->GetTransactionId(tuple_slot);
//      (void)tid123;
...
//  LOG_TRACE("Aborting peloton txn : %" PRId64, current_txn->GetTransactionId());
  ...
  for (const auto &tuple_entry : rw_set) {
    ...
    if (tuple_entry.second == RWType::READ_OWN) {
      ...
      if(index_entry_ptr != nullptr){
        //没有索引的删除无法回滚. 现在这里加一个判断条件. 然后再看看效果.
        UNUSED_ATTRIBUTE auto res = AtomicUpdateItemPointer(
            index_entry_ptr, ItemPointer(tile_group_id, tuple_slot));
        PELOTON_ASSERT(res == true);
      }
...
ResultType TimestampOrderingTransactionManager::AbortTransaction(
    TransactionContext *const current_txn, bool is_entire, bool is_fail_aft_del) {
  // a pre-declared read-only transaction will never abort.
  PELOTON_ASSERT(!current_txn->IsReadOnly());
//  LOG_TRACE("Aborting peloton txn : %" PRId64, current_txn->GetTransactionId());
  ...
  if(!is_entire&&!is_fail_aft_del){
    return ResultType::ABORTED;
  }else if (!is_entire&&is_fail_aft_del){
//  }else if (is_fail_aft_del){//测试用
    //如果是更新主键中的先删除再插入后报错了, 那就回滚rw_set的第一条
    //todo rw_set是一个无序的集合, 没有办法拿到它最后存放的那个.
    ItemPointer item_ptr = current_txn->GetDelLocation();
    uint tile_group_id = item_ptr.block;
    uint tuple_slot = item_ptr.offset;
