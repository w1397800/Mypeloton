...
#include "storage/peloton/concurrency/epoch_manager.h"
#include "storage/peloton/concurrency/local_epoch.h"
...
public:
  ...
  virtual void SetCurrentTxnId(const uint32_t txn_id) override {
    next_txn_id_ = txn_id;
  }
...
//    thread_pool.SubmitDedicatedTask(&DecentralizedEpochManager::Running, this);
   ...
   */
  virtual cid_t EnterEpoch(const size_t thread_id, const TimestampType1 ts_type) override;
  ...
  /**
   * @brief      Gets the current txn identifier.
   *
   * @return     The current txn identifier.
   */
  virtual uint32_t GetCurrentTxnId() override{
