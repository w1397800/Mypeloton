...
#ifndef A3264212_E28F_4E99_9C1F_E7765BA506E9
#define A3264212_E28F_4E99_9C1F_E7765BA506E9
...
class MetricTimer {
   public:
    MetricTimer(std::string metric_name) : _metric_name(metric_name) {}
    ~MetricTimer() = default;

    void start() {
        _start_time_us = butil::monotonic_time_us();
        if (_last_report_time_us == 0) {
            _last_report_time_us = _start_time_us;
        }
    }
    void end() {
        int64_t end_time_us = butil::monotonic_time_us();
        int64_t cost_time_us = end_time_us - _start_time_us;
        int64_t interval = end_time_us - _last_report_time_us;
        if (interval > 1000 * 1000 * _report_interval_s) {
            std::cout << "<METRIC REPORT> " << _metric_name
                      << " cost time: " << cost_time_us << " us" << '\n';
            _last_report_time_us = end_time_us;
        }
    }

   private:
    int64_t _start_time_us;
    int64_t _last_report_time_us{0};
    int64_t _report_interval_s = 2;
...
#endif /* A3264212_E28F_4E99_9C1F_E7765BA506E9 */
