...
struct PeerId {
    ...
    std::string get_addr_str() const{
        char str[128];
        snprintf(str, sizeof(str), "%s", butil::endpoint2str(addr).c_str());
        return std::string(str);
    }
...
// A set of peers.
// the configuration in the raft algorithm.
...
public:
    ...
    explicit ClusterConfiguration(const std::vector<PeerId>& peers) {
    ...
    explicit ClusterConfiguration(const std::set<PeerId>& peers) : _peers(peers) {}
    ...
    bool equals(const ClusterConfiguration& rhs) const {
        if (size() != rhs.size()) {
            return false;
        }
    ...
    void diffs(const ClusterConfiguration& rhs,
               ClusterConfiguration* included,
               ClusterConfiguration* excluded) const {
        *included = *this;
        *excluded = rhs;
...
std::ostream& operator<<(std::ostream& os, const ClusterConfiguration& a);
