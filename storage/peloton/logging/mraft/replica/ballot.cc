...
int Ballot::init(const ClusterConfiguration& conf,
    ...
    for (ClusterConfiguration::const_iterator iter = conf.begin();
         iter != conf.end(); ++iter) {
...
Ballot::PosHint Ballot::grant(const PeerId& peer, PosHint hint) {
    ...
    iter = _find_peer(peer, _old_peers, hint.pos1);
