...
#include "rpc/local_storage.pb.h"
...
DEFINE_int32(raft_max_segment_size, 128 * 1024 * 1024 /*128M*/,
...
int Segment::load(ConfigurationManager* configuration_manager) {
    ...
    for (int64_t i = _first_index; entry_off < file_size; i++) {
        ...
        if (header.type == ENTRY_TYPE_CONFIGURATION) {
            LOG(FATAL) << "Configuration entry fuction not implemented";
            break;
        }
...
int Segment::append(const std::shared_ptr<LogEntry> entry) {
...
std::shared_ptr<LogEntry> Segment::get(const int64_t index) const {
    LogMeta meta;
...
int SegmentLogStorage::append_entries(
...
int SegmentLogStorage::append_entry(const std::shared_ptr<LogEntry> entry) {
    scoped_refptr<Segment> segment = open_segment();
    if (NULL == segment) {
        return EIO;
    }
...
std::shared_ptr<LogEntry> SegmentLogStorage::get_entry(const int64_t index) {
...
scoped_refptr<Segment> SegmentLogStorage::open_segment() {
    scoped_refptr<Segment> prev_open_segment;
    {
        ...
        if (_open_segment->bytes() > FLAGS_raft_max_segment_size) {
            LOG(WARNING) << "FLAGS_raft_max_segment_size: "
                         << FLAGS_raft_max_segment_size;
