...
Value NumericFunctions::Sqrt(const std::vector<Value> &args) {
...
// Get Abs of value
Value NumericFunctions::_Abs(const std::vector<Value> &args) {
  PELOTON_ASSERT(args.size() == 1);
  if (args[0].IsNull()) {
    return ValueFactory::GetNullValueByType(TypeId::DECIMAL);
  }
  switch (args[0].GetElementType()) {
    case TypeId::DECIMAL: {
      ...
      return ValueFactory::GetDecimalValue(result);
    }
    case TypeId::INTEGER: {
      int32_t result;
      result = abs(args[0].GetAs<int32_t>());
      return ValueFactory::GetIntegerValue(result);
    }
    case TypeId::BIGINT: {
      int64_t result;
      result = std::abs(args[0].GetAs<int64_t>());
      return ValueFactory::GetBigIntValue(result);
    }
    case TypeId::SMALLINT: {
      int16_t result;
      result = abs(args[0].GetAs<int16_t>());
      return ValueFactory::GetSmallIntValue(result);
    }
    case TypeId::TINYINT: {
      int8_t result;
      result = abs(args[0].GetAs<int8_t>());
      return ValueFactory::GetTinyIntValue(result);
    }
    default: {
      return ValueFactory::GetNullValueByType(TypeId::DECIMAL);
    }
...
Value NumericFunctions::_Ceil(const std::vector<Value> &args) {
  PELOTON_ASSERT(args.size() == 1);
  if (args[0].IsNull()) {
    return ValueFactory::GetNullValueByType(TypeId::DECIMAL);
  }
...
Value NumericFunctions::_Floor(const std::vector<Value> &args) {
  PELOTON_ASSERT(args.size() == 1);
  if (args[0].IsNull()) {
    return ValueFactory::GetNullValueByType(TypeId::DECIMAL);
  }
...
Value NumericFunctions::_Round(const std::vector<Value> &args) {
  PELOTON_ASSERT(args.size() == 1);
  if (args[0].IsNull()) {
    return ValueFactory::GetNullValueByType(TypeId::DECIMAL);
  }
  return ValueFactory::GetDecimalValue(Round(args[0].GetAs<double>()));
}
...
//    UNUSED_ATTRIBUTE const codegen::Type &type, const char *ptr,
//    uint32_t len) {
