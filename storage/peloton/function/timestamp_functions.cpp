...
#include "storage/peloton/type/limits.h"
...
uint64_t TimestampFunctions::DateTrunc(const char *date_part_type,
  ...
  if (timestamp == PELOTON_TIMESTAMP_NULL) {
    return PELOTON_TIMESTAMP_NULL;
  }
  ...
  switch (date_part) {
    ...
    default: { result = PELOTON_TIMESTAMP_NULL; }
  };

  return (result);
}

Value TimestampFunctions::_DateTrunc(
  ...
  if (timestamp == PELOTON_TIMESTAMP_NULL) {
    return ValueFactory::GetNullValueByType(TypeId::TIMESTAMP);
  }

  uint64_t ret = DateTrunc(date_part, timestamp);
  return ValueFactory::GetTimestampValue(ret);
}
...
double TimestampFunctions::DatePart(const char *date_part_type,
  ...
  switch (date_part) {
    ...
    default: { result = PELOTON_DECIMAL_NULL; }
  };

  return (result);
}

Value TimestampFunctions::_DatePart(
    const std::vector<Value> &args) {
  char *date_part = args[0].GetAs<char *>();
  ...
  return ValueFactory::GetDecimalValue(ret);
}
