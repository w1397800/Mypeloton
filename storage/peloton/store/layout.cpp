...
Layout::Layout(const uint num_columns, LayoutType layout_type)
...
Layout::Layout(const column_map_type &column_map)
  ...
  for (uint column_id = 0; column_id < num_columns_; column_id++) {
...
Layout::Layout(const column_map_type &column_map, const uint num_columns,
               const uint layout_id)
...
void Layout::LocateTileAndColumn(uint column_id, uint &tile_id,
                                 uint &tile_column_id) const {
...
double Layout::GetLayoutDifference(const Layout &other) const {
  double theta = 0;
  double diff = 0;
...
//  PELOTON_ASSERT(this->num_columns_ == other.num_columns_);
  ...
  if ((this->layout_oid_ != other.layout_oid_)) {
    for (uint col_itr = 0; col_itr < num_columns_; col_itr++) {
...
uint Layout::GetTileIdFromColumnId(uint column_id) const {
...
uint Layout::GetTileColumnOffset(uint column_id) const {
  uint tile_column_id, tile_offset;
  LocateTileAndColumn(column_id, tile_offset, tile_column_id);
  return tile_column_id;
}

std::vector<Schema> Layout::GetLayoutSchemas(
    Schema *const schema) const {
  std::vector<Schema> schemas;

  // Build the schema tile at a time
  std::map<uint, std::vector<Column>> tile_schemas;

  // Handle schemas population based on the layout type
  if (layout_type_ == LayoutType::ROW) {
    schemas.push_back(*schema);
  } else if (layout_type_ == LayoutType::COLUMN) {
    for (uint col_id = 0; col_id < num_columns_; col_id++) {
      std::vector<Column> tile_schema({schema->GetColumn(col_id)});
    ...
    for (auto entry : tile_schemas) {
      Schema tile_schema(entry.second);
      schemas.push_back(tile_schema);
    }
  }
...
std::map<uint, uint> Layout::GetLayoutStats() const {
  std::map<uint, uint> column_map_stats;
...
column_map_type Layout::DeserializeColumnMap(uint num_columns,
                                             std::string column_map_str) {
  column_map_type column_map;
  std::stringstream ss(column_map_str);

  for (uint col_id = 0; col_id < num_columns; col_id++) {
    uint str_col_id, tile_id, tile_col_id;
...
std::string Layout::GetColumnMapInfo() const {
  std::stringstream ss;
  std::map<uint, std::vector<uint>> tile_column_map;
  ...
  if (layout_type_ == LayoutType::ROW) {
    ...
    for (auto column_info : column_layout_) {
      uint col_id = column_info.first;
      uint tile_id = column_info.second.first;
