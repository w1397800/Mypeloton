...
class LogicalTile  {
 ...
 public:
  ...
  typedef std::vector<uint> PositionList;
  ...
  void AddColumn(const std::shared_ptr<Tile> &base_tile,
                 uint origin_column_id, uint position_list_idx);

  void AddColumns(const std::shared_ptr<TileGroup> &tile_group,
                  const std::vector<uint> &column_ids);

  void ProjectColumns(const std::vector<uint> &original_column_ids,
                      const std::vector<uint> &column_ids);

  int AddPositionList(PositionList &&position_list);

  void RemoveVisibility(uint tuple_id);

  Tile *GetBaseTile(uint column_id);
  ...
  const ColumnInfo &GetColumnInfo(const uint column_id) const;

  Schema *GetPhysicalSchema() const;

  void SetSchema(std::vector<LogicalTile::ColumnInfo> &&schema);

  const PositionLists &GetPositionLists() const;

  const PositionList &GetPositionList(const uint column_id) const;
  ...
  std::unique_ptr<Tile> Materialize();
   ...
   */
  class iterator : public std::iterator<std::input_iterator_tag, uint> {
 ...
 private:
  ...
  void MaterializeColumnAtATime(
      ...
      const std::unordered_map<Tile *, std::vector<uint>>
      &tile_to_cols,
      Tile *dest_tile);
  ...
  void MaterializeByTiles(
      ...
      &tile_to_cols,
      Tile *dest_tile,
      const LayoutType peloton_layout_mode = LayoutType::ROW);
