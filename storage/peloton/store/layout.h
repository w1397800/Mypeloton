 ...
 */
typedef std::map<uint, std::vector<std::pair<uint, uint>>> TileToColumnMap;
...
class Layout {
 public:
   ...
   */
  Layout(const uint num_columns, LayoutType layout_type = LayoutType::ROW);
   ...
   */
  Layout(const column_map_type &column_map, const uint num_columns,
         const uint layout_oid);
  ...
  uint GetOid() const { return layout_oid_; }
   ...
   */
  void LocateTileAndColumn(uint column_id, uint &tile_id,
   ...
   */
  double GetLayoutDifference(const Layout &other) const;

  /** @brief  Returns the tile_id of the given column_id. */
  uint GetTileIdFromColumnId(uint column_id) const;

  /** @brief  Returns the column offset of the column_id. */
  uint GetTileColumnOffset(uint column_id) const;
  ...
  static column_map_type DeserializeColumnMap(uint num_columns,
