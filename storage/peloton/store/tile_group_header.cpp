...
const std::string TileGroupHeader::GetInfo() const {
    ...
    uint active_tuple_slots = GetCurrentNextTupleSlot();
    for (uint header_itr = 0; header_itr < active_tuple_slots; header_itr++) {
        ...
        // NEXT RANGE
        ItemPointer nextPointer = GetNextItemPointer(header_itr);
        os << "Next:[";
        ...
        // PREVIOUS RANGE
        ItemPointer prevPointer = GetPrevItemPointer(header_itr);
        os << "Prev:[";
...
void TileGroupHeader::PrintVisibility(txn_id_t txn_id, cid_t at_cid) {
    ...
    for (uint header_itr = 0; header_itr < active_tuple_slots; header_itr++) {
        ...
        ItemPointer location = GetNextItemPointer(header_itr);
        os << " prev : "
...
// operations.
uint TileGroupHeader::GetActiveTupleCount() const {
    uint active_tuple_slots = 0;
