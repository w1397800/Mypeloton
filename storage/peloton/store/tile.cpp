...
Tile::Tile(BackendType backend_type, TileGroupHeader *tile_header,
  ...
  if(FOR_NVM){
    data_nvm = new char[tile_size];//nvm
  }
  ...
  pool = new EphemeralPool();//3.11
//  pool = new IntelPool();//3.11
  //}
}
 ...
 */
void Tile::InsertTuple(const uint tuple_offset, Tuple *tuple) {
...
Value Tile::GetValue(const uint tuple_offset, const uint column_id) {
  PELOTON_ASSERT(tuple_offset < GetAllocatedTupleCount());
  ...
  return Value::DeserializeFrom(field_location, column_type, is_inlined);
}
...
Value Tile::GetValueFast(const uint tuple_offset,
 ...
 */
// column id is a 0-based column number
void Tile::SetValue(const Value &value, const uint tuple_offset,
  ...
  const TypeId col_type = schema.GetType(column_id);
  if (value.GetTypeId() == col_type) {
    value.SerializeTo(field_location, is_inlined, pool);
  } else {
    Value casted_value = value.CastAs(col_type);
 ...
 */
// column offset is the actual offset of the column within the tuple slot
void Tile::SetValueFast(const Value &value, const uint tuple_offset,
...
Tile *Tile::CopyTile(BackendType backend_type) {
  ...
  if (!tile_columns_inlined) {
    ...
    for (uint col_itr = 0; col_itr < uninlined_col_cnt; col_itr++) {
      auto uninlined_col_offset = schema->GetUninlinedColumn(col_itr);

      // Copy the column over to the new tile group
      for (uint tuple_itr = 0; tuple_itr < allocated_tuple_count;
           tuple_itr++) {
        Value val = (new_tile->GetValue(tuple_itr, uninlined_col_offset));
...
bool Tile::SerializeTo(SerializeOutput &output, uint num_tuples) {
  /**
...
bool Tile::SerializeHeaderTo(SerializeOutput &output) {
  ...
  for (uint column_itr = 0; column_itr < column_count; ++column_itr) {
    TypeId type = schema.GetType(column_itr);
...
void Tile::DeserializeTuplesFrom(SerializeInput &input,
  ...
  uint column_count = input.ReadShort();
  PELOTON_ASSERT(column_count > 0);

  // Store the following information so that we can provide them to the user on
  // failure
//  TypeId types[column_count];
  std::vector<TypeId> types(column_count);
  std::vector<std::string> names;
  // Skip the column types
  for (uint column_itr = 0; column_itr < column_count; ++column_itr) {
    types[column_itr] = (TypeId)input.ReadEnumInSingleByte();
  }
...
// active tuple slots
uint Tile::GetActiveTupleCount() const {
  // For normal tiles
...
bool Tile::operator==(const Tile &other) const {
  ...
  Schema other_schema = other.schema;
...
//std::vector<std::vector<std::string>> Tile::GetAllValuesAsStrings(
