...
#include "storage/peloton/store/layout.h"
 ...
 */
Schema *LogicalTile::GetPhysicalSchema() const {
  ...
  Schema *schema = new Schema(physical_columns);
 ...
 */
void LogicalTile::RemoveVisibility(uint tuple_id) {
 ...
 */
Tile *LogicalTile::GetBaseTile(uint column_id) {
...
Value LogicalTile::GetValue(uint tuple_id, uint column_id) {
  PELOTON_ASSERT(column_id < schema_.size());
  PELOTON_ASSERT(tuple_id < total_tuples_);

  ColumnInfo &cp = schema_[column_id];
  uint base_tuple_id = position_lists_[cp.position_list_idx][tuple_id];
  Tile *base_tile = cp.base_tile.get();
...
void LogicalTile::SetValue(Value &value UNUSED_ATTRIBUTE,
                           uint tuple_id UNUSED_ATTRIBUTE,
                           uint column_id UNUSED_ATTRIBUTE) {
 ...
 */
uint LogicalTile::iterator::operator*() { return pos_; }
...
LogicalTile::PositionListsBuilder::PositionListsBuilder(
  ...
  output_lists_.push_back(std::vector<uint>());
 ...
 * before the metadata.
 */
void LogicalTile::AddColumn(const std::shared_ptr<Tile> &base_tile,
                            uint origin_column_id, uint position_list_idx) {
  ColumnInfo cp;
...
void LogicalTile::AddColumns(
  ...
  for (uint origin_column_id : column_ids) {
    uint base_tile_offset, tile_column_id;
 ...
 * new column_ids
 * column_ids is a vector of uint. Each column_id is the index into the
 * original table schema
 * schema_ is a vector of ColumnInfos. Each ColumnInfo represents a column in
 * the corresponding place in colum_ids.
 */
void LogicalTile::ProjectColumns(const std::vector<uint> &original_column_ids,
                                 const std::vector<uint> &column_ids) {
...
std::vector<std::vector<std::string>> LogicalTile::GetAllValuesAsStrings(
  ...
  for (uint tuple_itr = 0; tuple_itr < total_tuples_; tuple_itr++) {
    std::vector<std::string> row;
    std::string empty_string;
    if (visible_rows_[tuple_itr] == false) continue;
    for (uint column_itr = 0; column_itr < schema_.size(); column_itr++) {
      const LogicalTile::ColumnInfo &cp = schema_[column_itr];
      uint base_tuple_id = position_lists_[cp.position_list_idx][tuple_itr];
      // get the value from the base physical tile
      Value val;
      if (base_tuple_id == NULL_OID) {
        val = ValueFactory::GetNullValueByType(
...
void LogicalTile::GenerateTileToColMap(
    const std::unordered_map<uint, uint> &old_to_new_cols,
    std::unordered_map<Tile *, std::vector<uint>>
    &cols_in_physical_tile) {
  for (const auto &kv : old_to_new_cols) {
    uint col = kv.first;

    // figure out base physical tile for column in logical tile
    Tile *base_tile = GetBaseTile(col);

    std::vector<uint> &cols_from_tile = cols_in_physical_tile[base_tile];
...
void LogicalTile::MaterializeByTiles(
    const std::unordered_map<uint, uint> &old_to_new_cols,
    const std::unordered_map<Tile *, std::vector<uint>> &tile_to_cols,
    Tile *dest_tile,
    const LayoutType peloton_layout_mode) {
  ...
  uint column_count_threshold = 20;
...
void LogicalTile::MaterializeRowAtAtATime(
  ...
  for (const auto &kv : tile_to_cols) {
    const std::vector<uint> &old_column_ids = kv.second;
    ...
    std::vector<uint> old_column_position_idxs;
    std::vector<size_t> old_column_offsets;
    std::vector<TypeId> old_column_types;
    std::vector<bool> old_is_inlineds;
    std::vector<Tile *> old_tiles;
    ...
    for (uint old_col_id : old_column_ids) {
    ...
    for (uint old_tuple_id : *this) {
      ...
      for (uint old_col_id : old_column_position_idxs) {
        auto &column_position_list = column_position_lists[old_col_id];

        uint base_tuple_id = column_position_list[old_tuple_id];
...
void LogicalTile::MaterializeColumnAtATime(
  ...
  for (const auto &kv : tile_to_cols) {
    ...
    for (uint old_col_id : old_column_ids) {
      ...
      for (uint old_tuple_id : *this) {
        uint base_tuple_id = column_position_list[old_tuple_id];
        Value value = (old_tile->GetValueFast(
 ...
 */
std::unique_ptr<Tile> LogicalTile::Materialize() {
  // Create new schema according underlying physical tile
  std::unique_ptr<Schema> source_tile_schema(GetPhysicalSchema());

  // Get the number of tuples within this logical tiles
  const int num_tuples = GetTupleCount();

  // const Schema *output_schema;
  std::unordered_map<uint, uint> old_to_new_cols;
  uint column_count = source_tile_schema->GetColumnCount();
  for (uint col = 0; col < column_count; col++) {
    old_to_new_cols[col] = col;
  }

  // Generate mappings.
  std::unordered_map<Tile *, std::vector<uint>> tile_to_cols;
  GenerateTileToColMap(old_to_new_cols, tile_to_cols);

  // Create new physical tile.
  std::unique_ptr<Tile> dest_tile(
  TileFactory::GetTempTile(*source_tile_schema, num_tuples));
