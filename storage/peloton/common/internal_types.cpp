...
//      case StatementType::TRANSACTION: {
//        switch (
//            static_cast<const TransactionStatement *>(sql_stmt)->type) {
//          case TransactionStatement::CommandType::kBegin:
//            query_type = QueryType::QUERY_BEGIN;
//            break;
//          case TransactionStatement::CommandType::kCommit:
//            query_type = QueryType::QUERY_COMMIT;
//            break;
//          case TransactionStatement::CommandType::kRollback:
...
//      }
//      case StatementType::CREATE: {
//        switch (static_cast<const CreateStatement *>(sql_stmt)->type) {
...
std::string TimestampTypeToString(TimestampType1 type) {
  switch (type) {
    case TimestampType1::INVALID: {
      return "INVALID";
    }
    case TimestampType1::SNAPSHOT_READ: {
      return "SNAPSHOT_READ";
    }
    case TimestampType1::READ: {
      return "READ";
    }
    case TimestampType1::COMMIT: {
      return "COMMIT";
    }
    default: {
      // throw ConversionException(StringUtil::Format(
...
TimestampType1 StringToTimestampType(const std::string &str) {
  std::string upper_str = StringUtil::Upper(str);
  if (upper_str == "INVALID") {
    return TimestampType1::INVALID;
  } else if (upper_str == "SNAPSHOT_READ") {
    return TimestampType1::SNAPSHOT_READ;
  } else if (upper_str == "READ") {
    return TimestampType1::READ;
  } else if (upper_str == "COMMIT") {
    return TimestampType1::COMMIT;
  } else {
    // throw ConversionException(StringUtil::Format(
