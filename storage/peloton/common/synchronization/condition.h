...
#include "storage/peloton/common/synchronization/mutex_latch.h"
...
class Condition {
 public:
  ...
  bool TimedwaitRelative(const struct timespec &relative_time) {
    //PELOTON_ASSERT(0 <= relative_time.tv_nsec && relative_time.tv_nsec < ONE_S_IN_NS);
  ...
  bool Timedwait(const struct timespec &absolute_time) {
    //PELOTON_ASSERT(0 <= absolute_time.tv_nsec &&absolute_time.tv_nsec < ONE_S_IN_NS);
