...
#define INVALID_LOG_FILE_NUMBER -1
...
/* 傻逼 xa.h 引用后会产生冲突导致编译不成功
 * 所以自己定义了一个跟原始 xid_t 一样的结构，在接口处转换就行 */
struct PXID {
 private:
  /**
 ...
 public:
  PXID() : formatID(-1), gtrid_length(0), bqual_length(0) {
...
enum LogRecordType {
  ...
  LOGRECORD_TYPE_TRANSACTION_PREPARE = 6,
  ...
  //Checkpoint records
  LOGRECORD_TYPE_CP_BEGIN = 61,
  LOGRECORD_TYPE_CP_END = 62,
};
std::string LogRecordTypeToString(LogRecordType type);
LogRecordType StringToLogRecordType(const std::string &str);
...
//static const size_t ACTIVE_COUNT = 30;
//static const size_t MT_PS_COUNT = 30;
...
static const size_t GC_THD_COUNT = 10;
//====================wjh
...
// 简易版的检查点文件，用于恢复时打开文件，以及对文件排序
struct CheckpointFile {
  uint version_number;
  std::string file_name;
  CheckpointFile() : version_number(0), file_name(nullptr){}
  CheckpointFile(int number, std::string name) : version_number(number), file_name(std::move(name)){}
};
...
TypeId PostgresValueTypeToPelotonValueType(PostgresValueType type);
