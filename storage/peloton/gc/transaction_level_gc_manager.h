...
#include "storage/peloton/common/init.h"
...
#include "storage/peloton/common/container/lock_free_queue.h"
 ...
 public:
  TransactionLevelGCManager(const int thread_count)
    ...
    for (int i = 0; i < gc_thread_count_; ++i) {
      std::shared_ptr<LockFreeQueue<TransactionContext* >>
          unlink_queue(new LockFreeQueue<TransactionContext* >(
              MAX_QUEUE_LENGTH));
  ...
  virtual void StartGC() override {
    ...
    this->is_running_ = true;
    for (int i = 0; i < gc_thread_count_; ++i) {
      new std::thread(&TransactionLevelGCManager::Running, this, std::move(i));
...
//                                      std::move(i));
  ...
  virtual void RecycleTransaction(TransactionContext *txn) override;

  virtual ItemPointer ReturnFreeSlot(const uint &table_id) override;

  virtual void RegisterTable(const uint &table_id) override {
  ...
  virtual void DeregisterTable(const uint &table_id) override {
 ...
 private:
  ...
  void AddToRecycleMap(TransactionContext *txn_ctx);
  ...
  void UnlinkVersions(TransactionContext *txn_ctx);
 ...
 private:
  ...
  std::vector<std::shared_ptr<LockFreeQueue<TransactionContext* >>> unlink_queues_;

  // local queues for to-be-unlinked tuples.
  // # local_unlink_queues == # gc_threads
  std::vector<std::list<TransactionContext* >> local_unlink_queues_;
  ...
  std::vector<std::multimap<cid_t, TransactionContext* >> reclaim_maps_;

  // queues for to-be-reused tuples.
  // # recycle_queue_maps == # tables
  std::unordered_map<uint, std::shared_ptr<LockFreeQueue<ItemPointer>>> recycle_queue_map_;
};
