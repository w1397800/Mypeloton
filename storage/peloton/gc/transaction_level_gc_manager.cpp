...
void TransactionLevelGCManager::RecycleTransaction(
  ...
  if (!txn->IsReadOnly() && \
    ...
    txn->SetEpochId(epoch_manager.GetNextEpochId());  //??? abort事务不是应该立刻回收吗？
  }
...
int TransactionLevelGCManager::Unlink(const int &thread_id,
  ...
  eid_t safe_expired_eid =
      EpochManagerFactory::GetInstance().GetCurrentEpochId();
...
void TransactionLevelGCManager::AddToRecycleMap(
  ...
  for (auto &entry : *(txn_ctx->GetGCSetPtr().get())) {
    ...
    DataTable *table =
        dynamic_cast<DataTable *>(tile_group->GetAbstractTable());
    PELOTON_ASSERT(table != nullptr);

    uint table_id = table->GetOid();
    auto tile_group_header = tile_group->GetHeader();
    ...
    for (auto &element : entry.second) {
      ...
      //该case是导致垃圾回收产生duplicate key的问题所在.
      if ((!immutable) && recycle_queue_map_.find(table_id) != recycle_queue_map_.end()) {
        if(!(table_id == 1 || table_id == 2||table_id == 0)){
  ...
  for (auto &entry : *(txn_ctx->GetGCObjectSetPtr().get())) {
    uint database_oid = std::get<0>(entry);
    uint table_oid = std::get<1>(entry);
    uint index_oid = std::get<2>(entry);
...
ItemPointer TransactionLevelGCManager::ReturnFreeSlot(const uint &table_id) {
...
void TransactionLevelGCManager::UnlinkVersion(const ItemPointer location,
  ...
  auto tile_group =
      StorageManager::GetInstance()->GetTileGroup(location.block);
  ...
  auto tile_group_header =
      StorageManager::GetInstance()->GetTileGroup(location.block)->GetHeader();
  ...
  ContainerTuple<TileGroup> current_tuple(tile_group.get(),
                                                   location.offset);
...
//
//      // build key.
//      std::unique_ptr<Tuple> current_key(
//          new Tuple(index_schema, true));
